...
import { InputBoolean, isNotNil } from './convert';

@Component({
    selector: 'nz-basetree',
    templateUrl: './nz-tree.component.html',
    styleUrls: ['./nz-tree.scss'],
    providers: [
        NzTreeService,
        {
...
export class NzTreeComponent implements OnInit, OnChanges, OnDestroy {
    ...
    myExpandChange(event) {
        const node = event.node;
        this.levels.splice(node.level + 1);
        if (node.children && node.children.length) {
            this.levels[node.level + 1] = node.children;
        }
    }
